...
<%Calendar cal = Calendar.getInstance();
			...
			//		cal.setTimeInMillis(date값에서 getTimeMillis잇어요);  이건 date에서 string 파싱하는
				...
				<div class="section_1" id="reserve">
					<ul>
						<li>
							<h3>티 켓 선 택 / 매 수 선 택</h3> <select name="concertName">
								<c:forEach var="list" items="${concerts}">
									<option value="${list.concertName}">${list.concertName}</option>
								</c:forEach>
					...
					<ul>
						...
						<li>
							...
							name="reservationCost" size="5" style="text-align: right;" />원 <a
							href="#"><input type="button" value="예매하기"
								onclick="javascript:window.open('sign.jsp','new','width=500,height=350')">예매하기</a> -->

							<input type="text" name="reservationCost" size="5"
							style="text-align: right;" />원 <input type="submit" value="예매하기">예매하기</a>
						</li>
					</ul>
				</div>
