...
public class UserManageController extends HttpServlet {
		...
		if (action != null) {
			if (action.equals("register")) {
				...
				if (request.getSession().getAttribute("userid") == "admin") {
					pushUserlistPage(request, response);
				}
			}
		}
	}

	private void pushUserlistPage(HttpServletRequest request,
	...
	protected void doPost(HttpServletRequest request,
		...
		if (requestUrl.equals(WPConstants.PAGE_SERVER_URL
				+ WPConstants.PAGE_URL_USER_LOGIN)) {
			...
			forwarding(request, response, confirm);
				...
				+ WPConstants.PAGE_URL_USER_MODIFY)) {
			confirm = pushModify(request, response);
			forwarding(request, response, confirm);
		} else if (requestUrl.equals(WPConstants.PAGE_SERVER_URL
				+ WPConstants.PAGE_URL_USER_SIGNUP)) {
